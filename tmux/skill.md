...
```bash
SOCKET_DIR=${TMPDIR:-/tmp}/amp-tmux-sockets  # well-known dir for all agent sockets
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/amp.sock"                # keep agent sessions separate from your personal tmux
SESSION=amp-python                           # slug-like names; avoid spaces
...
To monitor this session yourself:
  tmux -S "$SOCKET" attach -t amp-lldb

Or to capture the output once:
  tmux -S "$SOCKET" capture-pane -p -J -t amp-lldb:0.0 -S -200
```
...
- Agents MUST place tmux sockets under `AMP_TMUX_SOCKET_DIR` (defaults to `${TMPDIR:-/tmp}/amp-tmux-sockets`) and use `tmux -S "$SOCKET"` so we can enumerate/clean them. Create the dir first: `mkdir -p "$AMP_TMUX_SOCKET_DIR"`.
- Default socket path to use unless you must isolate further: `SOCKET="$AMP_TMUX_SOCKET_DIR/amp.sock"`.

## Targeting panes and naming

- Target format: `{session}:{window}.{pane}`, defaults to `:0.0` if omitted. Keep names short (e.g., `amp-py`, `amp-gdb`).
...
- Scan all sockets under the shared directory: `./scripts/find-sessions.sh --all` (uses `AMP_TMUX_SOCKET_DIR` or `${TMPDIR:-/tmp}/amp-tmux-sockets`).
...
- Capture full scrollback (entire history): `tmux -S "$SOCKET" capture-pane -p -S - -t target`.
...
Chain multiple tmux commands in a single invocation using `';'` (quoted to avoid shell interpretation):

```bash
tmux -S "$SOCKET" new-window -n "$SESSION" -d ';' send-keys -t "$SESSION" "npm start" C-m
```
